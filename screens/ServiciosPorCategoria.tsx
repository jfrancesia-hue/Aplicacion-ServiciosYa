...
import { getBlockedUserIds, isUserBlockedByMe } from "../lib/utils/trustSafety";
import { createUrgentWorkAlert } from "../lib/utils/urgentWorkNotification";
...
function WorkerCard({
  worker,
  onPress,
}: { worker: Worker; onPress: () => void }) {
  const ubicacion = [worker.ciudad, worker.provincia]
    .filter(Boolean)
    .join(", ");
          ...
          {worker.antiguedad != null && (
            <View style={styles.metaRow}>
              ...
              <Text style={styles.metaText}>
                {worker.antiguedad} años de experiencia
              </Text>
            </View>
          )}
        </View>
...
function WorkerDetailModal({
  ...
  const compartirPerfil = async () => {
    ...
    try {
      await Share.share({
        message,
        url,
        title: nombre || "Profesional Servicios Ya",
      });
    } catch {
  ...
  const contactarChat = async (urgent = false) => {
    ...
    try {
      ...
      if (urgent) {
        const urgentBody = `Un cliente necesita una respuesta urgente por ${worker.categoria?.[0] || "un servicio"}. Aceptá o rechazá dentro de 20 minutos.`;
        await createUrgentWorkAlert({
          ...
          body: urgentBody,
          metadata: { worker_nombre: worker.nombre },
        });
      }
          ...
          <View style={styles.modalBox}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              >
                ...
                {!!worker.edad && (
                  <Text style={styles.modalAge}>{worker.edad} años</Text>
                )}
              </LinearGradient>
              <View style={styles.modalContent}>
                {!!ubicacion && (
                  <View style={styles.modalRow}>
                    <MaterialIcons
                      ...
                      color="#069eb3"
                    />
                    <Text style={styles.modalRowText}>{ubicacion}</Text>
                  </View>
                )}
                <View style={styles.modalRow}>
                  <MaterialIcons
                ...
                {worker.antiguedad != null && (
                  <View style={styles.modalRow}>
                    <MaterialIcons name="work" size={18} color="#069eb3" />
                    <Text style={styles.modalRowText}>
                      {worker.antiguedad} años de experiencia
                    </Text>
                  </View>
                ...
                {Array.isArray(worker.categoria) &&
                  worker.categoria.length > 0 && (
                  ...
                  <View style={{ marginBottom: 4 }}>
                    ...
                    {workerServices.map((svc) => (
                      <View key={svc.id} style={styles.svcCard}>
                        ...
                        {!!svc.descripcion && (
                          <Text style={styles.svcDesc}>{svc.descripcion}</Text>
                        )}
                          ...
                          {svc.precio != null && (
                            <View style={styles.svcMetaItem}>
                              ...
                              <Text style={styles.svcMetaText}>
                                ${svc.precio.toLocaleString("es-AR")}
                              </Text>
                            </View>
                          )}
                          {!!svc.horario && (
                ...
                <TouchableOpacity
                  ...
                  onPress={() => void contactarChat(false)}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => void contactarChat(true)}
                >
                  <MaterialIcons name="bolt" size={21} color="#a43b32" />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.urgentBtnText}>
                      Necesito respuesta urgente
                    </Text>
                    <Text style={styles.urgentBtnHint}>
                      Debe aceptar o rechazar dentro de 20 minutos
                    </Text>
                  </View>
                </TouchableOpacity>
              </View>
...
type UserLocation =
  | { ciudad?: string; provincia?: string; localidad?: string }
  | null
  | undefined;
            ...
            {locationError ? (
              ...
              >
                <View style={styles.scopeIcon}>
                  <MaterialIcons
                    name={
                      locationFallbackActive ? "travel-explore" : "my-location"
                    }
                    size={18}
                    color="#fff"
                  />
...
const styles = StyleSheet.create({
  ...
  urgentBtnText: { color: "#a43b32", fontSize: 15, fontWeight: "800" },
  urgentBtnHint: { color: "#76514c", fontSize: 11, marginTop: 2 },
  shareBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
