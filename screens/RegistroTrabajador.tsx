...
export default function RegistroTrabajadorSimplificado() {
    ...
    try {
      if (tipo === "imagen") {
        ...
        if (!result.canceled && result.assets.length > 0) {
          ...
          setLista([
            ...lista,
            { uri: asset.uri, tipo: "pdf", nombre: asset.name },
          ]);
        }
      }
    } catch (e) {
  ...
  const subirListaArchivos = async (lista: SelectedFile[]) => {
    ...
    for (const archivo of lista) {
      const contentType =
        archivo.tipo === "pdf" ? "application/pdf" : "image/jpeg";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await recordCurrentLegalAcceptance("provider_registration");
              ...
              <View style={styles.modalContainer}>
                <View style={styles.modalBox}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={elegirContratar}
                    disabled={procesandoModal}
                  >
                    <Text style={styles.modalButtonText}>
                      {procesandoModal ? "Procesando..." : "Contratar"}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
            ...
            <View style={styles.overlay}>
              ...
              <View style={styles.basicInfoCard}>
                ...
                <Text style={styles.basicInfoText}>
                  Con estos datos ya podrás aparecer en tu zona. La
                  documentación es opcional.
                </Text>
              </View>
              ...
              />
              <TextInput
                placeholder="Número de celular (con código de país)"
                placeholderTextColor="#4e827d"
                value={numeroCelular}
              ...
              {mostrarOpcionales && (
                <View style={styles.optionalSection}>
                  <Text style={styles.optionalIntro}>
                    Estos datos ayudan a generar confianza, pero no son
                    obligatorios para estar disponible.
                  </Text>
                  ...
                  <Text style={styles.label}>
                    Matrícula (opcional, hasta 3 archivos)
                  </Text>
                  ...
                  <View style={styles.archivosPreview}>
                    {matriculaArchivos.map((a, i) => (
                      <View
                        key={`${a.uri}-${a.nombre}`}
                        style={styles.archivoChip}
                      >
                        <Text style={styles.archivoChipText} numberOfLines={1}>
                          {a.nombre}
                        </Text>
                        <TouchableOpacity
                          onPress={() =>
                            setMatriculaArchivos(
                              matriculaArchivos.filter((_, idx) => idx !== i),
                            )
                          }
                        >
                          <Text style={styles.tagClose}>✕</Text>
              ...
              >
                <Text style={styles.buttonText}>
                  {loading ? "Publicando..." : "Publicar perfil básico"}
                </Text>
              </TouchableOpacity>
            </View>
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  optionalSection: {
    ...
    padding: 14,
    borderRadius: 16,
    backgroundColor: "rgba(244,251,251,0.88)",
    borderWidth: 1,
    borderColor: "#d3e7e9",
  },
  optionalIntro: {
